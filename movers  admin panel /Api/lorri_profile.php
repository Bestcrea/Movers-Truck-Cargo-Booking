...
if($data['owner_id'] == '' or $data['lorry_id'] == '')
{
...
else
{
	...
	$getroutes = $service->query("select routes from tbl_lorry where owner_id=".$owner_id." and id=".$lorry_id."")->fetch_assoc();
	$state = $service->query("select GROUP_CONCAT(  DISTINCT CONCAT(title,',',img)SEPARATOR ';')  as fav_routes   from tbl_state where id IN(".$getroutes['routes'].")")->fetch_assoc();
	$apro['total_routes'] = explode(';',$state['fav_routes']);
