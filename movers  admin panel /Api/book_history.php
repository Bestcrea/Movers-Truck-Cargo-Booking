...
else
{
	...
	if($status == 'Current')
	{
	$list = $service->query("select id,lorry_id,lorry_owner_id,weight,pick_lat,pick_lng,drop_lat,drop_lng from tbl_load where uid=".$uid." and load_status!='Completed' and load_type='FIND_LORRY' order by id desc");
	}
	else 
	{
		$list = $service->query("select id,lorry_id,lorry_owner_id,weight,pick_lat,pick_lng,drop_lat,drop_lng from tbl_load where uid=".$uid." and load_status='Completed' and load_type='FIND_LORRY' order by id desc");
	...
	$vdata = $service->query("select title,img from tbl_vehicle where id=".$lorrydata['vehicle_id']."")->fetch_assoc();
		$odata = $service->query("select name,pro_pic from  tbl_lowner where id=".$row['lorry_owner_id']."")->fetch_assoc();
		$k['lorry_id'] = $row['id'];
		$k['vehicle_id'] = $lorrydata['vehicle_id'];
		$k['lorry_owner_id'] = $row['lorry_owner_id'];
		...
		$rdata_rest = $service->query("SELECT sum(total_trate)/count(*) as rate_rest FROM tbl_load where lorry_owner_id=".$row['lorry_owner_id']." and load_status='Completed' and total_trate !=0")->fetch_assoc();
